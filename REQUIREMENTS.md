...
### Enterprise Torpedo — 8×8, frame 0 (pointing right)
```
........
##......
..######
...
### Klingon Torpedo — 8×8, frame 4 (pointing left)
```
....###.
..###...
####....
...
*   **Planet:** A central animated circle containing horizontal scanlines. The planet sprite cycles through **16 animation states**, advancing one state every **16 game ticks** (visible only when the Planet toggle is ON). Rendered at **48 virtual pixels wide** (1.5× horizontal scale via Bresenham 3:2 mapping from the 32-bit source bitmap) and approximately **48 screen pixels tall** (Y_SCALE=2 applied to all 32 rows), giving a circular appearance on the 640×480 display. In attract mode the planet is drawn at a smaller 1:1 scale in the top-right corner.
...
*   **Explosions (Ship/Hyperspace):** The ship's hull disintegrates into **32 individual pixel particles** that scatter outward with random velocities. Ship death reuses the same particle system as hyperspace (see §5.4). The game remains in play mode for the duration of the explosion animation (~40 ticks / `SHIP_EXPLOSION_TICKS`); it transitions to Attract Mode only once the last particle has faded (object reaches `EFLG_INACTIVE`).
...
*   **Initial Velocity:** Torpedo velocity = ship's current velocity + firing direction vector scaled by **4x** (`FIRE_SCALE = 2`, left-shifted twice). Torpedo spawns offset from ship centre along the firing axis — the spawn offset is `cos/sin(angle) >> 11` (maximum ~15 virtual pixels), which is intentionally larger than `SHIP_TO_TORP_RANGE = 8` to prevent the torpedo from immediately colliding with the firing ship.
...
*   **Destination:** A random position is chosen at the moment the key is pressed and stored. The animation always travels from source to destination.
*   **Animation (64 ticks total, `HYPER_DURATION = 64`):**
    1.  **Phase 1 — Expansion (ticks 1–32, `HYPER_PHASE = 32`):** Ship disappears. **32 particles** (`HYPER_PARTICLES`) scatter outward from the ship's position. Each particle carries a **translational velocity component** (source → destination over `HYPER_DURATION` ticks) plus a **radial velocity component** (outward scatter). The cloud therefore expands while simultaneously drifting toward the halfway point between source and destination.
    2.  **Phase 2 — Contraction (ticks 33–64):** At tick 33 each particle's velocity is recalculated so it converges from its current position to the destination in exactly `HYPER_PHASE` more ticks. The cloud contracts onto the destination point.
    3.  **Tick 64:** Particles deactivate, ship reappears at the pre-chosen destination.
*   **On Arrival:** Ship spawns at the destination. **All velocity is zeroed** (ship arrives stationary).
*   **Death Explosion vs Real Hyperspace:** Both use the same 32-particle system. A death explosion (triggered by `handle_death`) sets `ship.exps = SHIP_EXPLOSION_TICKS > 0` before the animation starts. At tick `HYPER_PHASE + 1` the code checks `ship.exps > 0` to distinguish death (expansion only, no contraction, no teleport) from a real jump (two-phase travel + teleport).
...
2.  Ship decomposes into **32 pixel particles** with random velocities, reusing the hyperspace particle system (`SHIP_EXPLOSION_TICKS = 40`). Particles expand outward only — no contraction phase, no teleport.
3.  Opponent's score increments by 1.
4.  **The game remains in play mode** while the explosion runs. Transition to **Attract Mode** occurs only once `ship.eflg` reaches `EFLG_INACTIVE` (all particles settled). Scores are preserved.
